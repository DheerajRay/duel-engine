...
export default function GameHistoryPage({ onBack }: { onBack: () => void }) {
                  ...
                  >
                    <div className="min-w-0">
                      <div className="text-[10px] font-mono uppercase tracking-[0.26em] text-zinc-500">
                        {entry.mode.replace('_', ' ')} · {entry.result}
                      </div>
                  ...
                  {expanded && (
                    <div className="border-t border-zinc-800 px-4 py-4 space-y-4">
                      ...
                      <div>
                        ...
                        <div className="mt-2 text-sm text-zinc-300">{entry.notablePlay}</div>
                      </div>

                      <div>
                        <div className="text-[10px] font-mono uppercase tracking-[0.24em] text-zinc-500 mb-2">Logs</div>
                        <div className="max-h-64 overflow-y-auto border border-zinc-800 bg-black">
                          {entry.logs.map((log) => (
                            <div key={log.id} className="border-b border-zinc-900 px-3 py-2 text-xs text-zinc-300 last:border-b-0">
                              {log.message}
                            </div>
                          ))}
                        </div>
